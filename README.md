...
> 🛡️ **브라우저 확장 프로그램 버전**
> 본 프로젝트는 기존 Electron 데스크탑 앱에서 Chrome Extension(Manifest V3)으로 성공적으로 전환되었습니다. 이제 브라우저 사이드 패널(Side Panel)을 통해 실시간 접근성 진단 및 판정 기능을 제공합니다.

## 프로젝트 개요
ABT(A11Y Browser Tester)는 웹 접근성 전문가를 위한 진단 도구입니다. 브라우저 사이드 패널에서 현재 탭의 접근성 진단 데이터를 실시간으로 수집, 시각화하며 전문가 판정 및 리포트 생성을 지원합니다.
