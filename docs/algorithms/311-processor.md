# ABT-7.1.1-Processor: 기본 언어 표시 알고리즘 v1.0

### 🔗 KWCAG 7.1.1 Reference
- **3.1.1 (기본 언어 표시)**: 주로 사용하는 언어를 명시해야 한다.

---

이 알고리즘은 **KWCAG 7.1.1(기본 언어 표시)** 지침을 준수하며, 웹 페이지의 최상위 요소(`<html>` 등)에 문서가 주로 사용하는 기본 언어가 올바르고 명확하게 정의되어 있는지 진단한다.
