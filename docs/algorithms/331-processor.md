...
1. **부적절 (Warning):**
   - 전문가가 제공된 가이드라인에 따라 폼을 수동 검사한 결과, 오류 유형 1~4 중 하나라도 발생한 경우.
2. **검토 필요 (Review) [Default State]:**
   - 시스템이 폼 요소를 감지하여 전문가에게 수동 검사를 요청한 대기 상태. (자동화 도구가 내리는 초기 상태)
3. **적절 (Pass):**
   - 전문가가 수동으로 에러 상황을 테스트해본 결과, 안내 텍스트 제공, 초점 이동, 입력 데이터 유지가 모두 완벽하게 동작하는 경우.
4. **N/A:**
   - 웹 페이지 내에 사용자가 데이터를 전송하는 입력 서식(Form) 요소가 아예 존재하지 않음.
