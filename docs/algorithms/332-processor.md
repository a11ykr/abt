# ABT-3.3.2-Processor: 표의 구성 알고리즘 v1.0
...
- **Rule 1.1:** `<caption>`이 없는가?
  - 결과: **[수정 권고]** (표의 제목 제공 필요)
- **Rule 1.2 (HTML5):** `summary` 속성이 존재하는가?
  - 결과: **[오류]** (HTML5에서 폐기된 속성. `aria-describedby` 또는 `caption` 내 상세 설명 권고)
...
1. **오류:** HTML5 `summary` 사용, 데이터 표 내 `th` 누락, 제목/요약 완전 누락.
2. **부적절:** 레이아웃 표에 `caption`/`th`를 사용하여 구조 혼란 유발.
3. **수정 권고:** `scope` 누락, "가짜 제목"(표 외부 텍스트) 사용.
