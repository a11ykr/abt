# ABT-8.1.1-Processor: 마크업 오류 방지 알고리즘 v1.0

### 🔗 KWCAG 2.2 Reference
- **8.1.1 (마크업 오류 방지)**: 웹 페이지의 콘텐츠는 마크업 오류가 없어야 한다.

---

이 알고리즘은 **KWCAG 8.1.1(마크업 오류 방지)** 지침을 준수하며, 웹 페이지의 HTML 마크업에 표준을 위반하는 치명적인 오류(태그 중첩 오류, 속성 중복 선언, ID 중복 등)가 없는지 진단한다. 경미한 경고성 오류보다는 보조기술의 해석에 직접적인 영향을 미치는 주요 오류를 중심으로 진단한다.
...
### [단계 B] 속성 중복 선언 검사
- **Rule 2.1:** `element.attributes`를 통해 각 요소의 속성을 분석했을 때, 동일한 속성명(예: `<div class="a" class="b">`)이 중복 선언된 경우가 존재하는가?
  - 결과: 속성 중복 선언 발견 시 **[오류]**.
